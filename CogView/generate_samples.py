...
def generate_images_once(model, args, raw_text, seq=None, num=8, query_template='{}', output_path='./samples'):
    ...
    with torch.no_grad():
        ...
        if args.debug:
            ...
            output_file = os.path.join(output_path, f"{output_file_prefix}-{datetime.now().strftime('%m-%d-%H-%M-%S')}.png")
            ...
            for i in range(len(imgs)):
                save_image(imgs[i], os.path.join(output_path,f'{i}.png'), normalize=True)
                os.chmod(os.path.join(output_path,f'{i}.png'), stat.S_IRWXO+stat.S_IRWXG+stat.S_IRWXU)
            save_image(torch.cat(imgs, dim=0), os.path.join(output_path,f'concat.png'), normalize=True)
            os.chmod(os.path.join(output_path,f'concat.png'), stat.S_IRWXO+stat.S_IRWXG+stat.S_IRWXU)
