...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids, boundary_ids=None):
        ...
        if is_torch_fx_proxy(input_ids):
            ...
            if boundary_ids != None:
                # Sicong: add boundary tokens
                shifted_input_ids = input_ids.new_zeros(input_ids.shape)
                shifted_input_ids[..., 52:] = input_ids[..., :-52].clone()
                shifted_input_ids[..., 0] = decoder_start_token_id
                shifted_input_ids[..., 1:51] = boundary_ids
                shifted_input_ids[..., 51] = decoder_start_token_id
            else:
                shifted_input_ids = input_ids.new_zeros(input_ids.shape)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
            ...
            decoder_input_ids = self._shift_right(labels, boundary_ids)
        ...
        # Sicong: do not consider the first output tokens (<s> <+> <->....)
        if boundary_ids != None:
            sequence_output = sequence_output[:,51:,:]
            if labels != None:
                labels = labels[:,:-51]
