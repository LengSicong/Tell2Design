...
from utils import get_precision_recall_f1, calculate_intersection, calculate_union, render_image, process_boundary_info, calculate_iou, render_image_
...
def miss_attributes(self,rooms_attributes):
    if self.data_args.exp == 'miss1attri_0': # miss aspect ratio
        ...
        for room in rooms_attributes:
            ...
            if rooms_attributes[room]['location'] != None:
                strings += (rooms_attributes[room]['location']+' ')
            if rooms_attributes[room]['private'] != None:
        ...
        raise Exception('wrong exp name about missing attributes experiments! miss1attri_0/miss1attri_1/miss1attri_2/miss1room/missrandom')
...
class FloorplanDataset(JointERDataset):
    ...
    def load_data_single_split(self, split: str, seed: int = None) -> List[InputExample]:
        ...
        with open(file_path, 'r') as f:
            ...
            for i, description in enumerate(data):
                ...
                near_x_min=room['near_x_min'], near_y_min=room['near_y_min'], near_x_max=room['near_x_max'], near_y_max=room['near_y_max'],
                relation=room['relation'],location=room['location'], size=str(room['size']), aspect_ratio=room['aspect ratio'],private=str(room['private'])) for j,room in enumerate(description['rooms'])]
                ...
                for rm in rooms:
                    rooms_attributes[rm.type] = {'location':None,'size':None,'aspect ratio':None,'size+aspect ratio':None,'private':None,'room_type':None}
                ...
                if self.data_args.exp.endswith('finetune'):
                    ...
                    if 'annotated_strings' in description:
                        ...
                        for room in rooms_attributes:
                            ...
                            # # with relation
                            # if rooms_attributes[room]['relation'] != None:
                            #     strings += (rooms_attributes[room]['relation']+' ')
                            # baseline version
                            if rooms_attributes[room]['location'] != None:
                                strings += (rooms_attributes[room]['location']+' ')
                            if rooms_attributes[room]['size+aspect ratio'] != None:
                                strings += (rooms_attributes[room]['size+aspect ratio']+' ')
    ...
    def evaluate_example(self, example: InputExample, output_sentence: str, model=None, tokenizer=None,output_dir=None, prediction=None) -> Counter:
        """
        Evaluate an output sentence on a single example of this dataset.
        """
        # extract entities and relations from output sentence
        ...
        for room in predicted_attri:
            predicted_boxes[room] = [[int(float(predicted_attri[room]['x coordinate'])-float(predicted_attri[room]['height'])/2), int(float(predicted_attri[room]['y coordinate'])-float(predicted_attri[room]['width'])/2)], [int(float(predicted_attri[room]['x coordinate'])+float(predicted_attri[room]['height'])/2), int(float(predicted_attri[room]['y coordinate'])-float(predicted_attri[room]['width'])/2)],
            [int(float(predicted_attri[room]['x coordinate'])-float(predicted_attri[room]['height'])/2), int(float(predicted_attri[room]['y coordinate'])+float(predicted_attri[room]['width'])/2)], [int(float(predicted_attri[room]['x coordinate'])+float(predicted_attri[room]['height'])/2), int(float(predicted_attri[room]['y coordinate'])+float(predicted_attri[room]['width'])/2)]
                        ]
