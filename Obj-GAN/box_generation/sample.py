...
parser.add_argument('--load_checkpoint', action='store', dest='load_checkpoint', 
    default=None, help='The name of the checkpoint to load, \
...
if opt.is_training and opt.load_checkpoint is None:
    # load vocab
    ...
    optimizer = None
    bidirectional = True
    if not opt.resume and opt.load_checkpoint is None:
        # Initialize model
    ...
    t = SupervisedTrainer(lloss=lloss, bloss=bloss, batch_size=opt.batch_size, 
        checkpoint_every=10000, print_every=100, expt_dir=opt.expt_dir, 
    ...
    seq2seq = t.train(encoder, decoder, train_tuples, num_epochs=20, dev_data=dev_tuples, 
        optimizer=optimizer, resume=opt.resume, is_training=opt.is_training)

    # Sicong: 2nd finetune
elif opt.is_training and opt.load_checkpoint is not None:
    logging.info("loading checkpoint from {}".format(os.path.join(opt.load_checkpoint)))
    checkpoint_path = os.path.join(opt.load_checkpoint)
    ...
    weight = torch.ones(len(label_word2index))
    for word in label_word2index:
        # sicong: label vocab may be different for 1st and 2nd finetune
        if word not in train_label_lang.word2count:
            train_label_lang.word2count[word] = 0
        if train_label_lang.word2count[word] == 0:
            continue
        index = label_word2index[word]
        weight[index] = weight[index]*opt.count_smooth/float(math.pow(
        	train_label_lang.word2count[word], 0.8))

    # Prepare loss
    pad = label_word2index["<pad>"]
    lloss = Perplexity(weight, pad, opt.lamda1)
    bloss = BBLoss(opt.batch_size, opt.gmm_comp_num, opt.lamda2)
    if torch.cuda.is_available():
        lloss.cuda()
    ...
    seq2seq = t.train(encoder, decoder, train_tuples, num_epochs=100, dev_data=dev_tuples, 
        optimizer=optimizer, resume=opt.resume, is_training=opt.is_training)
